...
export default {
  ...
  components: {},
  props: ['drillId', 'type', 'teamId', 'teamType'],
  data() {
    // 这里存放数据
    return {
      ruleForm: {
  ...
  created() {
    this.getTeams()
    if (this.teamType == '攻击方团队') {
      this.ruleForm.roleName = '攻击方'
    } else {
      this.ruleForm.roleName = '防御方'
    }
  },
  // 生命周期 - 挂载完成（可以访问DOM元素）
  mounted() {
    ...
    getTeams(_type) {
      ...
      pageTeam(that.listQuery).then((res) => {
        if (res.data.code == 200) {
          ...
          that.ruleForm.teamId = that.teamId
          // if (that.teamType == '攻击方团队') {
          //   console.log('sssssss');
          //   that.ruleForm.roleName = '攻击方'
          // } else {
          //   that.ruleForm.roleName = '防御方'
          // }
        }
      })
    },
    changeTeam(val) {
      const team = this.teamOptions.map(function(item) {
        if (item.id == val) {
          return item.name
        }
      })
      this.ruleForm.teamName = team[0].name
      this.resetForm.teamType = team[0].type
    },
    changeRole(val) {
      let type = ''
      if (val == '攻击方') {
