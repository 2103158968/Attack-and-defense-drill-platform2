<template>
  <div class="sys-body">
    ...
    <div class="top-container flex-wrap box-pack-justify">
      <div class="flex-wrap">
        <div class="sys-add-btn sys-btn" @click="addVisible = true"><i class="el-icon-add" />新增{{ tabValue }}
        </div>
    ...
    <div class="list-container flex-wrap flex-rowWrap">
      <div v-for="(item,index) in tableData" :key="index" class="list-item" @dblclick="editData(item)">
        <div class="flex-wrap">
          <div>
        ...
        <div class="flex-wrap box-align box-pack-justify btn-list">
          <div class="add-btn" @click.top="addUser(item.id)">添加成员</div>
    ...
    <pagination
      ...
      @pagination="getList"
    />
    <el-empty v-if="total == 0" description="暂无数据" :image-size="500" />
    <!-- 添加团队 -->
    <el-dialog :visible.sync="addVisible" :title="tabValue ==='攻击方团队'?'编辑攻击方团队':'编辑防御单位团队'" @close="handleClose">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="24">
            <el-form-item label="所属分组：" prop="groupId">
              <el-select v-model="ruleForm.groupId">
                <el-option v-for="(item,index) in groupOptions" :key="index" :value="item.id" :label="item.name" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="提交报告：" prop="submit">
              <el-upload
                class="upload-demo"
          ...
          <el-col :span="24">
            <el-form-item label="团队介绍：" prop="introduce">
              ...
              <el-input v-model="ruleForm.introduce" placeholder="团队介绍" type="textarea" :rows="4" resize="none" />
            </el-form-item>
          </el-col>
        </el-row>
...
import { pageTeam, oneTeam, saveTeam, delTeam } from '@/api/team'
import AddUser from '@/views/public/addUser.vue'
  ...
  methods: {
    ...
    submit(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          // alert('submit!');
          this.ruleForm.type = this.tabValue
          saveTeam(this.ruleForm).then(res => {
            if (res.data.code == 200) {
              this.$message({
                type: 'success',
